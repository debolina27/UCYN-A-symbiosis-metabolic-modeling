...
/*
this code finds the least number of metabolites that must be transferred between the two organisms for maximal UCYN-A N2 fixation flux
...
***CO2 limited growth 
LB('Exchange_cpd00011_ctobin[ctobin]') = -10.0;
...
obj_maxHostBio ..     z =e= v('Biomass_Chlamy_auto[ctobin]');

max_org_trans_cons..		sum(j$(org_trans(j)), ytrans(j)) =l= max_org_trans ;
maxHostBio_cons..				v('Biomass_Chlamy_auto[ctobin]') =g= maxHostBio;
...
FILE resultfile /"%out_path%FindHost2UCYNtransfer_maxHostBiomass.txt"/;
...
put "Maximizing UCYN-A N2 fixation"//;

Solve maximizeHostBiomass using mip maximizing z;
maxHostBio.fx = z.l;
put "maximal Host Biomass flux: ",z.l:0:10,"  ",maximizeHostBiomass.modelstat//;
