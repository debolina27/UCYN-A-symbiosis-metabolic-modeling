...
Integer cuts are used to find multiple solutions
*/
...
****number of integer cut iterations to perform
iter /1*3/
...
integercut(eqncounter)..   sum(j$(prev_y(j,eqncounter) eq 1),ytrans(j)) =l= sum(j,prev_y(j,eqncounter)) - 1;
...
FILE resultfile /"%out_path%FindHost2UCYNtransfer_UCYNsimultaneousBioProd_Icuts.txt"/;
...
***let us do integer cuts now
rerun_flag = 1;
loop(iter$(rerun_flag eq 1),
put //" Integer cuts iteration ",iter.tl,"  ",rerun_flag//;
...
**set the rerun flag
if ( ((pfba.modelstat eq 10.0) or (pfba.modelstat eq 4.0)  ),
rerun_flag = 0;);

***add anothe rlement to dynamic set
eqncounter(iter)=yes;

***Initiate prev_y for this iteration
prev_y(j,iter)=0;

loop(j1$(org_trans(j1) and (v.l(j1) ne 0.0)),
prev_y(j1,iter)=1;);
